...
try {
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        switch ($input['action']) {
            case 'create_task':
                if (!isset($input['title']) || !isset($input['description']) || !isset($input['priority']) || !isset($input['due_date'])) {
                ...
                $stmt->bind_param("ssiiss", $input['title'], $input['description'], $input['assigned_to'], $input['assigned_by'], $input['priority'], $input['due_date']);
                
                if ($stmt->execute()) {
                    // Log the task creation
                    $log_stmt = $conn->prepare("INSERT INTO activity_log (user_id, action, details) VALUES (?, ?, ?)");
                    $user_id = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : $input['assigned_by'];
                    $details = "Task created: " . $input['title'];
                    $log_stmt->bind_param("iss", $user_id, $input['action'], $details);
                    $log_stmt->execute();
            ...
            case 'update_status':
                if (!isset($input['task_id']) || !isset($input['new_status'])) {
                ...
                if ($stmt->execute()) {
                    ...
                    $details = "Task status changed to: " . $input['new_status'];
                    $log_stmt->bind_param("iss", $user_id, $input['action'], $details);
                    $log_stmt->execute();
